...
class Node:
    def __init__(self,purity,klasslabel='',pdistribution=[],score=0,wlearner=None):
        ...
        self.purity = purity
        self.fidx=-1
        self.params=0     # in case of linear and conic
...
class DecisionTree:
    ...
    def __init__(self, purity, exthreshold=5, maxdepth=10,
     weaklearner="Axis-Aligned-Random", pdist=False, nsplits=10, nfeattest=None):
        '''
        Input:
        -----------------
    ...
    def build_tree(self, X, Y, depth):
        ...
        if (nexamples==0):
            ...
            purity=prob[max_idx]          #Maximum Probability class
            node=Node(purity)

            if (purity>=self.purity or len(X)<self.exthreshold or depth==self.maxdepth):
                node.klasslabel=klasses[max_idx]
                print 'C(posterior=[],class = '+str((node.klasslabel))+',Purity = '+str(purity)  
                return node

            else:
                if (nexamples>self.exthreshold or purity<self.purity):

                    weak_obj=self.getWeakLearner()

                    split, mingain, Xlidx, Xridx = weak_obj.train(X,Y)

                    node.fidx=weak_obj.feat_idx
                    node.params=weak_obj.params
                    lC=X[Xlidx]
                    rC=X[Xridx]
                    Ymask=Y[Xlidx]
                    nMask=Y[Xridx]
                    node.split=split
                    node.score=mingain
                    node.wlearner=weak_obj

                    print 'Creating Left Child Node with '+str(len(lC))+' , and Right Node with '+str(len(rC))+ ' Examples'                       
                    node.lchild=self.build_tree(lC,Ymask,depth+1)                                    
                    node.rchild=self.build_tree(rC,nMask,depth+1)                  
    ...
    def _predict(self,node, X):
        ...
        pclass=[]
        trav_node=node
        while(not(trav_node.isleaf())):
            if (trav_node.isless_than_eq(X)):
                trav_node=trav_node.lchild
            else:
                trav_node=trav_node.rchild                  
        pclass.append(trav_node.klasslabel)
